// app.js
// UI orchestration: load CSV → prepare data → pretrain AE → train classifier → tune thresholds → evaluate & visualize.
// Designed for GitHub Pages (no server). Fixes previous callback error using tf.CustomCallback.
...
let model = null;
let accChart = null;
let tunedThresholds = null;

function setDisabled(el, v){ el.disabled = !!v; }
function setState(s){ // init | loaded | trained
  setDisabled(ui.train, s!=='loaded');
  setDisabled(ui.predict, s!=='trained');
  setDisabled(ui.save, s!=='trained');
}
...
function tuneThresholds(valPred, yVal, lo=0.30, hi=0.70, step=0.01){
  ...
  for (let j=0;j<D;j++){
    ...
    for (let thr=lo; thr<=hi+1e-9; thr+=step){
      ...
      const acc=c/N; if (acc>best){ best=acc; bestThr=+thr.toFixed(3); }
    }
    th[j]=bestThr;
  }
  return th;
...
async function handleTrain(){
  ...
  try {
    ...
    if (aeEpochs > 0) {
      await model.pretrainAE(ds.X_train, {
        epochs: aeEpochs, batchSize: batch, noiseStd: aeNoise
      }, (e, logs) => log(`AE Epoch ${e+1}/${aeEpochs} — mse=${logs.loss?.toFixed(6)}`));
    } else {
      log('AE pretraining skipped (epochs=0).');
    }

    // 2) Classifier training (freeze encoder then fine-tune)
    progress(60, 'Training classifier...');
    await model.fitClassifier(
    ...
    // Tune thresholds on validation (per output)
...
async function handlePredict(){
  ...
  try {
    ...
    const pred = await predT.array(); predT.dispose();
    const truth = await ds.y_test.array();

    const res = computePerStockMetrics(pred, truth, ds.symbols, ds.horizons, tunedThresholds||0.5);
    drawAccuracyBarChart(res);
    drawTimelines(res, ds.horizons, ds.baseDatesTest);
    drawConfusions(res);
    const overall = overallAccuracy(pred, truth, tunedThresholds||0.5);
    log(`Test accuracy ${(overall*100).toFixed(2)}% (tuned thresholds).`);
    progress(100,'Done');
  } catch (e){ console.error(e); alert(`Prediction error: ${e.message}`); log(`Error: ${e.message}`); progress(0,''); }
}

async function handleSave(){ if (!model) { alert('No trained model.'); return; } await model.save('tfjs_gru_ae'); log('Models downloaded.'); }

function init(){
  setState('init');
  ui.load.addEventListener('click', handleLoad);
  ui.train.addEventListener('click', handleTrain);
  ui.predict.addEventListener('click', handlePredict);
