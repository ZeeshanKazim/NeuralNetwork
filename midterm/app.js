/* =========================================================================
   Airline Passenger Satisfaction — Wide & Deep (TF.js, GitHub Pages)
   - Robust CSV load (Kaggle defaults), preview, missing%
   - Flexible column resolver (handles spaces/case variants)
   - Preprocess: median/mode impute, z-score numerics, one-hot cats
   - Optional engineered features: TotalDelay, Class×TypeOfTravel
   - Wide&Deep model: wide linear logit + deep MLP, sigmoid
   - Metrics: ROC/AUC, threshold slider → confusion + P/R/F1
   - Predict + export CSVs; save model
   ========================================================================= */

const S = {
  rawTrain: [], rawTest: [],
  map: null,            // column mapping + stats
...
// unify a row into canonical keys
function unifyRow(row){
  ...
  // normalize target as 0/1 if present
...
function buildMapping(trainRows){
  ...
  // Numeric columns (ratings + core). ArrivalDelay optional based on useCase.
  const NUM_KEYS_BASE = [
  ...
  for(const k of CAT_KEYS){
    ...
    catVals[k] = Array.from(new Set([...uniq, 'UNK'])); // include UNK
  }

  // Feature crosses (wide)
  let crossVals = [];
  ...
  // Deep numeric list (with optional engineered TotalDelay)
  const DEEP_NUM = [...NUM_KEYS];
  if(useTot){
    DEEP_NUM.push('totaldelay'); // derived
  }

  // Wide vector spec = one-hot for each categorical + cross
  const WIDE_SPEC = { cats:[...CAT_KEYS], cross:{enabled:useCross, key:crossKey, values:crossVals} };

  // Deep vector spec = z-scored numerics + (optionally) cats again (acts like embedding-lite)
...
function imputeAndVec(rows, M){
  ...
  for(let i=0;i<rows.length;i++){
    ...
    // Build WIDE vector (one-hots + cross)
    const w = [];
    for(const k of M.WIDE_SPEC.cats){
      const cats = M.catVals[k];
      const oh = cats.map(cv => rc[k]===cv ? 1 : 0);
      w.push(...oh);
    }
    ...
    // Build DEEP vector (z-numerics + optionally cats again)
    const d = [];
    for(const k of M.DEEP_SPEC.nums){
...
function modelSummaryText(m){
  const lines=[]; m.summary(undefined, undefined, s=>lines.push(s));
  return lines.join('\n');
}
...
async function onLoad(){
  try{
    ...
    $('fixNote').textContent= '—'; // airline CSV rarely needs “repairs”
...
function onPreprocess(){
  try{
    ...
    $('preInfo').textContent = info;
  }catch(e){
    console.error(e); alert('Preprocessing failed: '+(e?.message||e));
  }
}
...
async function onTrain(){
  try{
    ...
    console.error(e); alert('Training failed: '+(e?.message||e));
  }
}
function onStop(){ stopFlag=true; alert('Early stop requested (after this epoch).'); }
...
function onPredict(){
  try{
    ...
    console.error(e); alert('Prediction failed: '+(e?.message||e));
  }
}
